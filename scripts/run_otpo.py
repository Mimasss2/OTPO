...
from scripts.otpo_trainer import OTPOTrainer
...
@dataclass
class OTPOConfig(DPOConfig1):
    ot_reg: float = field(
        default=1,
        metadata={"help": ("Coefficient for transport plan regularization")}
    )
    ot_reg_m: Tuple = field(
        default=(0.2, ) ,
        metadata={"help": ("Coefficient for mass regularization")}
    )
...
LLAMA_CHAT_TEMPLATE = "{% for message in messages %}{% set content = message['content'] %}{% if message['role'] == 'user' %}{{ bos_token + '[INST] ' + content.strip() + ' [/INST]' }}{% elif message['role'] == 'assistant' %}{{ ' '  + content.strip() + ' ' + eos_token }}{% endif %}{% endfor %}"


def apply_chat_template(
    example,
    tokenizer,
    task: Literal["sft", "generation", "rm", "po"],
    auto_insert_empty_system_msg: bool = True,
    change_template = None,
):
    if change_template == "mistral":
        tokenizer.chat_template = MISTRAL_CHAT_TEMPLATE
    if change_template == 'llama':
...
def main():
    ...
    parser = H4ArgumentParser((ModelArguments, DataArguments, OTPOConfig))
